# 如何在[RIME](https://rime.im/)中使用超强快码输入法
...
这个文档就是如何在RIME中支持超强快码输入法。

## 超强快码输入法
超强快码、超强二笔、超强快码都是付东升老师基于二笔输入法改进而来的输入法。它们的官方网站[在此](http://fds8866.ys168.com/)。
...
### 自定义方法
自造词可以放在`cqkm_userdict.txt`中。

如果有字或者词希望排序在前，可以把该词放在`cqkm_user_movetop.txt`中。
...
把这些文件连同cqkm.schema.yaml、cqkm_pinyin.schema.yaml一起传到手机上（通过iRIME的“电脑快传”功能）。再像上面一样修改iRIME的`default.custom.yaml`就可以了。
