...
rv_var={ week_var="week",date_var="date",nl_var="nong",time_var="time",jq_var="jie",switch_keyword="next",help="help",switch_schema="kuai"}	--编码关键字修改
...
local function get_schema_name(schema_id)
	...
	if file then
		for line in file:lines() do
			local m,n=line:match("(%s*name%:%s)%p([^%c%s]+)%p")
			if m and n then
				return n
			end
		end
...
local function set_switch_keywords(input, seg,env)
	...
	local keyword={{"简体⇉繁体",""},{"繁体⇉简体",""},{"切换方案",""}}
	if input == rv_var.switch_keyword then
		if schema_name then segment.prompt =" 〔 当前方案："..schema_name.." 〕" end
		for i =1,#keyword do
			local cand = Candidate(input, seg.start, seg._end, keyword[i][1],keyword[i][2])
			cand.quality=100000000
			yield(cand)
		end
	elseif input == rv_var.switch_schema and #enable_schema_list>0 then
		for i =1,#enable_schema_list do
			if enable_schema_list[i][2] then
				local comment=""
				if enable_schema_list[i][1]==schema_id then comment="  💡" end
