		...
		for cand in input:iter() do
			...
			if cand.type == 'simplified' and env.name_space == 'new_for_rvlk' then
				if cand.comment=="" then
					...
					yield(Candidate("new_spelling", cand.start, cand._end, cand.text, comment))
				end
			else
				if script_text:find("^[a-z]*([%`])[a-z%`]*") and not script_text:find("%p$") or script_text:find("^([%/])[a-z]*") and not script_text:find("%p$") then
					...
					if add_comment~=nil or add_comment~="" then
						if cand.comment == "" then
							yield(Candidate("new_spelling", cand.start, cand._end, cand.text,add_comment))
						else
							if cand.comment:find("☯") then
								yield(cand)
								-- yield(Candidate("new_spelling", cand.start, cand._end, cand.text,add_comment .. cand.comment))
							else
								if utf8.len(cand.text) == 1 and code_comment and not hide_pinyin then
									yield(Candidate("new_spelling", cand.start, cand._end, cand.text,xform(code_comment:gsub('%[(.-),(.-),(.-)%]', '[%1'..' · '..'%2'..' · '..'%3]'))))
								else
									yield(Candidate("new_spelling", cand.start, cand._end, cand.text,add_comment:gsub("〕"," · ") .. cand.comment .. " 〕"))
								end
							end
						end
					end
